    ...
    actions: {
        ...
        putObject: {
            async handler(ctx) {
                ...
                let params = {
                    bucketName: owner,
                    objectName: _.get(ctx.meta,"store.objectName"),
                    metaData: _.get(ctx.meta,"store.metData",{})
                };
                ...
                function getHeader() {
                    ...
                    return new Promise((resolve, reject) => {
                        self.client.getPartialObject(bucketName, ctx.params.objectName, 0, 116, (err, dataStream) => {
                            if (err) {
                                this.logger.warn("Failed to retrieve header of file", { err: err });
                                reject(err);
                            }
        ...
        listBuckets: {
            async handler(ctx) {
                let service = _.get(ctx.meta,"acl.service",null);
                if (service !== "admin") throw new Error("access not authorized");
