        ...
        getObject: {
            ...
            async handler(ctx) {
                    ...
                    return new Promise((resolve, reject) => {
                        self.client.getPartialObject(bucketName, ctx.params.objectName, 0, 116, (err, dataStream) => {
                            ...
                            dataStream.on("data", function(chunk) {
                                ...
                                this.logger.debug("Header stream: data", { bucket: bucketName, object: ctx.params.objectName, header: header });
                            });
                            dataStream.on("close", function() {
                                this.logger.debug("Header stream: close", { bucket: bucketName, object: ctx.params.objectName, header: header });
                                return resolve(header);
                            });
                            dataStream.on("finish", function() {
                                this.logger.debug("Header stream: finish", { bucket: bucketName, object: ctx.params.objectName, header: header });
                                return resolve(header);
                            });
                            dataStream.on("error", function(err) {
                                this.logger.debug("Header stream: error", { bucket: bucketName, object: ctx.params.objectName, error: err });
                                return reject(err);
                            });
                        }); 
                    });
                }
                this.logger.debug("Try to retrieve header", { bucket: bucketName, object: ctx.params.objectName });
                let header = await getHeader();
                this.logger.debug("Retrieved header", { bucket: bucketName, object: ctx.params.objectName });
                ...
                try {
                    ...
                    //
                    this.logger.debug("Failed to create decipher", { bucket: bucketName, object: ctx.params.objectName, err: err });
                }
                
                // create unzip stream
                let unzip = zlib.createUnzip();
                
                this.logger.info("Start streaming", { bucket: bucketName, object: ctx.params.objectName });
                encrypted = await this.client.getPartialObject(bucketName, ctx.params.objectName, 116);
                return encrypted
                    .pipe(decipher)
                    .pipe(unzip)
                    .on("error", (err) => self.logger.warn("Error streaming", { bucket: bucketName, object: ctx.params.objectName, err: err }));
